...
DEPS=(
  ...
  "wafw00f:pip:wafw00f"
  "whatweb:system:whatweb"
  # Email / Cloud OSINT
  "theharvester:system:theharvester"
...
ensure_tmux() {
  ...
  esac || { err "Failed to install tmux."; return 1; }
  ok "tmux installed."
}
...
_binary_name() {
  case "$1" in
    nmap_dns) echo "nmap" ;;
    email_harvest) echo "theHarvester" ;;
    cloud_enum) echo "cloud-enum" ;;
    *)        echo "$1"   ;;
  esac
}
...
setup() {
  ...
  mkdir -p "$OUTPUT_DIR"/{subdomains,dns,ports,http,content,urls,vulns,screenshots,takeover,emails,cloud}
...
apply_profile() {
  ...
  case "$profile" in
    ...
    quick)
      ...
      # OSINT / Cloud
      ENABLE_EMAIL_HARVEST=0; ENABLE_CLOUD_ENUM=0
    ...
    standard)
      ...
      ENABLE_EMAIL_HARVEST=1; ENABLE_CLOUD_ENUM=1
...
http_probe() {
  ...
  use whatweb && run "whatweb — tech fingerprinting" \
    bash -c "whatweb -i '$out/live_hosts.txt' -v > '$out/whatweb.txt' 2>/dev/null || true"
...
email_harvest() {
  ...
  local out="$OUTPUT_DIR/emails"

  if [ "${ENABLE_EMAIL_HARVEST:-0}" -eq 0 ]; then
    warn "Email harvesting disabled — enable from the tools menu or set ENABLE_EMAIL_HARVEST=1"
    return
  fi

  if ! has theHarvester; then
    warn "theHarvester not installed — skipping (install via ./install.sh)"
    return
  fi

  run "theHarvester — public email discovery" \
    bash -c "theHarvester -d '$DOMAIN' -b all 2>/dev/null | tee '$out/theharvester.txt' >/dev/null || true"

  grep -Eoi '[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}' \
    "$out/theharvester.txt" 2>/dev/null | sort -u > "$out/emails.txt" || true

  ok "Emails found : $(linecount "$out/emails.txt")"
  ok "Results → $out/"
}

cloud_enum() {
  section "CLOUD ENUMERATION"
  local out="$OUTPUT_DIR/cloud"

  if [ "${ENABLE_CLOUD_ENUM:-0}" -eq 0 ]; then
    warn "Cloud enumeration disabled — enable from the tools menu or set ENABLE_CLOUD_ENUM=1"
    return
  fi

  local ce_bin=""
  if has cloud-enum; then
    ce_bin="cloud-enum"
  else
    warn "cloud_enum not installed — skipping (install via ./install.sh)"
    return
  fi
  ...
  if [ -n "${CLOUD_ENUM_KEYWORDS:-}" ]; then
    local extra
    IFS=',' read -r -a extra <<< "$CLOUD_ENUM_KEYWORDS"
    keywords+=("${extra[@]}")
  fi
  ...
  run "cloud_enum — cloud asset discovery" \
    "$ce_bin" "${args[@]}" -l "$out/cloud_enum.txt" -f text

  local bucket_list="$out/buckets.txt"
  grep -Eo '([a-z0-9][a-z0-9.-]{1,61}[a-z0-9])\.s3[.-][a-z0-9.-]*amazonaws\.com' \
    "$out/cloud_enum.txt" 2>/dev/null \
    | sed 's/\.s3.*$//' | sort -u > "$bucket_list" || true

  if [ -s "$bucket_list" ]; then
    if has s3scanner; then
      run "s3scanner — bucket permission checks" \
        bash -c "s3scanner -bucket-file '$bucket_list' -json > '$out/s3scanner.json' 2>/dev/null || true"
    else
      warn "s3scanner not installed — skipping bucket permission checks"
    fi
  else
    warn "No S3 bucket candidates found from cloud_enum output"
  fi

  ok "Cloud assets : $(linecount "$out/cloud_enum.txt")"
  ok "Results → $out/"
}
...
custom_recon() {
  local modules=(
    ...
    "Email Harvesting (OSINT):email_harvest"
    "Cloud Enumeration:cloud_enum"
...
generate_summary() {
  ...
  # ── whatweb summary ───────────────────────────────────────────────────────
  local _whatweb_count
  _whatweb_count=$(linecount "$OUTPUT_DIR/http/whatweb.txt")

  # ── OSINT / Cloud summary ─────────────────────────────────────────────────
  local _emails_count _cloud_count _s3_count
  _emails_count=$(linecount "$OUTPUT_DIR/emails/emails.txt")
  _cloud_count=$(linecount "$OUTPUT_DIR/cloud/cloud_enum.txt")
  _s3_count=$(linecount "$OUTPUT_DIR/cloud/s3scanner.json")
...

  {
    ...
    printf "  %-34s %s\n" "whatweb lines:"          "$_whatweb_count"
    printf "  %-34s %s\n" "Emails found:"           "$_emails_count"
    printf "  %-34s %s\n" "Cloud assets found:"     "$_cloud_count"
    printf "  %-34s %s\n" "S3 checks (lines):"      "$_s3_count"
    [ "$_failed_count" -gt 0 ] && \
...
menu() {
  while true; do
    ...
    echo -e "   ${YELLOW}8${NC}  JS Secret Scanning"
    echo -e "   ${YELLOW}9${NC}  Email Harvesting (OSINT)"
    echo -e "  ${YELLOW}10${NC}  Cloud Enumeration"
    echo -e "  ${YELLOW}11${NC}  Screenshots"
    echo -e "  ${YELLOW}12${NC}  Subdomain Takeover Check"
    echo ""
...
usage() {
  ...
  echo "  CLOUD_ENUM_KEYWORDS=acme,corp      # extra keywords for cloud enum"
...
main() {
  # ── Always run inside tmux (auto-create session) ────────────────────────────
  if [ -z "${TMUX:-}" ] && [ -z "${RECON_TMUX:-}" ] && ! _should_skip_tmux "$@"; then
    ensure_tmux || exit 1
    local session; session=$(_tmux_session_name "$@")
    local args=()
    for a in "$@"; do args+=("$(printf '%q' "$a")"); done
    tmux new-session -d -s "$session" \
      "bash -lc 'cd \"$SCRIPT_DIR\"; RECON_TMUX=1 ./Rec0n-Zer0.sh ${args[*]}'"
