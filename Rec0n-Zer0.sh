...
#   go      — go install -v <value>
#   pip3    — pip3 install --user <value>
DEPS=(
  # Subdomain Enumeration
  ...
  "nikto:system:nikto"
  # Secret Scanning (go install works on all platforms)
...
install_deps() {
  ...
  # ── 6. Set GOPATH into tools directory and add Go bin to PATH ─────────────
  export GOPATH="$TOOLS_DIR/go"
  local go_bin="$GOPATH/bin"
  ...
  for dep in "${DEPS[@]}"; do
    ...
    if has "$tool"; then
      printf "  ${GREEN}[OK ]${NC}  %-18s already installed\n" "$tool"
    ...
    case "$method" in
      ...
      go)
        ...
        GOPROXY=https://proxy.golang.org,direct GONOSUMDB='*' GOPATH="$TOOLS_DIR/go" \
          go install -v "$value" 2>&1 | tail -3 || install_ok=0
        [ -d "$go_bin" ] && [[ ":$PATH:" != *":$go_bin:"* ]] && export PATH="$go_bin:$PATH" || true
        ;;
      pip3)
        if [ -x "$RECON_VENV/bin/pip" ]; then
          "$RECON_VENV/bin/pip" install --quiet "$value" 2>&1 | tail -3 || install_ok=0
        else
          # Fallback: --break-system-packages bypasses PEP 668 on Python 3.11+
          pip3 install "$value" --break-system-packages 2>&1 | tail -3 || install_ok=0
        fi
        ;;
      *)
...
check_deps() {
  ...
  for dep in "${DEPS[@]}"; do
    ...
    if has "$bin"; then
      local tool_path; tool_path=$(command -v "$bin")
...
apply_profile() {
  ...
  case "$profile" in
    ...
    quick)
      ...
      # Ports
      ENABLE_NAABU=0;      ENABLE_NMAP=1;      ENABLE_RUSTSCAN=0;  ENABLE_MASSCAN=0
      # HTTP
    ...
    standard)
      ...
      ENABLE_NAABU=1;      ENABLE_NMAP=1;      ENABLE_RUSTSCAN=1;  ENABLE_MASSCAN=0
...
port_scan() {
  ...
  use rustscan && run "rustscan — ultra-fast scan + nmap pass-through" \
    bash -c "rustscan -a '$target' --ulimit 5000 -b 1000 -- -sV -sC -oN '$out/rustscan.txt' 2>/dev/null || true"
