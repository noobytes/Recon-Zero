...
# Python packages required in venv (pip method tools from DEPS above)
PIP_PKGS=(
  "dnsrecon"
  "wafw00f"
)
...
install_pip_pkgs() {
  ...
  for pkg in "${PIP_PKGS[@]}"; do
    if "$RECON_VENV/bin/pip" install --quiet "$pkg" 2>/dev/null; then
      printf "  ${GREEN}[OK ]${NC}  %-20s installed (venv)\n" "$pkg"
      pip_ok=$((pip_ok + 1))
    else
      printf "  ${RED}[FAIL]${NC} %-20s failed\n" "$pkg"
      pip_fail=$((pip_fail + 1))
    fi
  done
  ok "pip packages: $pip_ok installed, $pip_fail failed"
}
...
setup_go() {
  ...
  # Point GOPATH at the tools directory so binaries land there
  export GOPATH="$TOOLS_DIR/go"
  local go_bin="$GOPATH/bin"
  mkdir -p "$go_bin" 2>/dev/null || true
...
verify_install() {
  ...
  if [ -d "$RECON_VENV" ] && [ -x "$RECON_VENV/bin/python3" ]; then
    ok "Python venv   : $RECON_VENV ($(\"$RECON_VENV/bin/python3\" --version 2>&1))"
  else
