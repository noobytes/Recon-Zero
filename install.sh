...
install_all_tools() {
  ...
  for dep in "${DEPS[@]}"; do
    ...
    # For go tools, prefer TOOLS_DIR, but skip if a system binary already exists.
    ...
    case "$method" in
      go)
        if [ -x "$TOOLS_DIR/go/bin/$tool" ]; then
          already_installed=1
          tool_path="$TOOLS_DIR/go/bin/$tool"
        elif has "$tool"; then
          already_installed=1
    ...
    if [ "$FORCE_INSTALL" -eq 0 ] && [ "$already_installed" -eq 1 ]; then
      [ -z "$tool_path" ] && tool_path="$(command -v "$tool" 2>/dev/null || true)"
      printf "  ${GREEN}[OK ]${NC}  %-18s already installed → %s\n" "$tool" "${tool_path:-unknown}"
...
usage() {
  ...
  echo "  -f, --force    Reinstall tools even if already present"
  ...
  echo "  ./install.sh --force        # reinstall all tools"
