...
Automated penetration testing reconnaissance suite for authorized security engagements.
...
> **Disk space:** Go tool compilation (`naabu`, `nuclei`, `trufflehog`, `gitleaks`) requires ~2 GB of free space. The disk check is run against the partition where the script lives. If builds fail with "no space left on device", free space and re-run. You can reclaim the Go build cache with `go clean -cache`.
...
> **Some tools** (`rustscan`, `feroxbuster`, `findomain`) are not in standard Debian repos but are available on Kali Linux. If the `system` install fails for a tool, it will be listed in the summary — install it manually or ensure you are running Kali.
